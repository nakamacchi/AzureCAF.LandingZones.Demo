...
下記の EDR 関連ポリシーは MDfC 側からデータが報告されないため、Azure Policy 側では not compliant 扱いになってしまう。このため、割り当て時のパラメータを調整（Disabled 化）するとよい。
...
内容としては以下でカバーされている。(EPP = EDR + Update Management)
...
ASB 内部でのパラメータ名は以下の 2 つ。既定値では下方互換性のため AuditIfNotExist になるが、Disabled にするとよい。（カッコ内は日本語名）
