...
# Defender for Cloud 脆弱性アセスメントの有効化
# https://learn.microsoft.com/en-us/azure/defender-for-cloud/sql-azure-vulnerability-assessment-enable
az rest --method PUT --uri "/subscriptions/${SUBSCRIPTION_ID_SPOKE_F}/resourcegroups/${TEMP_RG_NAME}/providers/microsoft.sql/servers/${TEMP_SQL_SERVER_NAME}/SecurityAlertPolicies/Default?api-version=2017-03-01-preview" --body @- <<EOF
{
  "properties": {
    "disabledAlerts": [
      ""
...
TEMP_RESOURCE_IDS="\
/subscriptions/${SUBSCRIPTION_ID_SPOKE_F}/resourceGroups/${TEMP_RG_NAME}/providers/Microsoft.Sql/servers/${TEMP_SQL_SERVER_NAME}
/subscriptions/${SUBSCRIPTION_ID_SPOKE_F}/resourceGroups/${TEMP_RG_NAME}/providers/Microsoft.ContainerRegistry/registries/${TEMP_ACR_NAME}
"

# PE を作成する VNET/Subnet
TEMP_PE_RG_NAME="rg-spokef-${TEMP_LOCATION_PREFIX}"
TEMP_PE_VNET_NAME="vnet-spokef-${TEMP_LOCATION_PREFIX}"
...
# ======================================
# 運用NW (vnet-spokefmtn-XXX) への PE 作成
...
# PE を作成する VNET/Subnet
TEMP_PE_RG_NAME="rg-spokefmtn-${TEMP_LOCATION_PREFIX}"
TEMP_PE_VNET_NAME="vnet-spokefmtn-${TEMP_LOCATION_PREFIX}"
TEMP_PE_SUBNET_NAME="PrivateEndpointSubnet"
az network vnet subnet update --name "${TEMP_PE_SUBNET_NAME}" --vnet-name $TEMP_PE_VNET_NAME --resource-group $TEMP_PE_RG_NAME --disable-private-endpoint-network-policies
