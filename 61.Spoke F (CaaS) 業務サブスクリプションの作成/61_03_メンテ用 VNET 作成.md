...
なお、ACA（Azure Container Apps）を VNET に組み込む場合、Web Apps や SQL DB などとは異なる技術を利用します（プライベートエンドポイントやリージョナル VNET 統合機能などを利用しません）。ACA の設定は Azure Portal から行うため、運用 NW 側から ACA への直接のアクセス経路は不要です。
