...
**2024/01/26 時点の注記：Defender CSPM のサブオプションである Permissions Management (Preview) は、現状では user_gov_change の持つ権限では有効化できません。下記スクリプトの EntraPermissionsManagement の有効化が必要な場合は admin 権限で行ってください。**
