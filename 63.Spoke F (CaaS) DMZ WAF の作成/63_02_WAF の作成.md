...
az network public-ip create --resource-group $TEMP_RG_NAME --name $TEMP_WAF_IP_NAME --allocation-method Static --sku Standard --dns-name "${TEMP_CA_NAME}-${UNIQUE_SUFFIX}"

az network application-gateway create --name $TEMP_WAF_NAME --location $TEMP_LOCATION_NAME --resource-group $TEMP_RG_NAME --capacity 2 --sku Standard_v2 --http-settings-cookie-based-affinity Enabled --public-ip-address $TEMP_WAF_IP_NAME --vnet-name $TEMP_VNET_NAME --subnet "DmzSubnet" --servers ${TEMP_CA_HOSTNAME} --priority 1
