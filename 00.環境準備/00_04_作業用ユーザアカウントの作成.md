...
**なお、対象アカウントに対して MFA が強制されている場合には、本サンプルの様々なところで行っている、スクリプトによるアカウント切り替えがうまく機能しません。この場合には手作業でログイン処理を行うか、一時的に対象アカウントに対する MFA 強制を外したうえで作業を進めてください。** 具体的には、以下を変更します。

- Entra ID Conditional Access > Policies から、MFA 強制を行っているポリシーを探す
- 該当ポリシーの適用対象ユーザを、All users などから System Administrator のみなどに変更する
