...
新規に作成したユーザアカウントに対して MFA（多要素認証）が必要な状態になっていると、以降のスクリプトでのアカウント切り替えのつど MFA 認証を求められ、作業が非常に煩雑になります。条件付きアクセスによって、「Azure管理アプリへのアクセスにMFAを強制する」のオプションが設定されているとこの状態になる場合がありますのでご注意ください。（この場合は当該オプションを外した状態で検証作業を進めてください。）
