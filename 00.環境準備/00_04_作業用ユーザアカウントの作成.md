...
本サンプルにおけるアカウント切り替え用のスクリプトは以下の通りです。特に利用する必要はありませんが、参考までに掲載します。**なお、対象アカウントに対して MFA が強制されている場合には、本サンプルの様々なところで行っている、スクリプトによるアカウント切り替えがうまく機能しません。この場合には手作業でログイン処理を行うか、一時的に対象アカウントに対する MFA 強制を外したうえで作業を進めてください。**
