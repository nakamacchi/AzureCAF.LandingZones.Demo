...
スクリプト実行の注意点として、**カスタムロールの削除処理（本スクリプトの最後の部分）は、実行後、完全に削除されるまで 10 分程度かかります**。これはカスタムロールの実装の一部にキャッシュが使われており、削除処理が各サーバに反映されるまでにタイムラグが発生するためです。スクリプト実行は一度だけで大丈夫ですが、しばらくはカスタムロールが消えたり復活したりするように見える点に注意してください。
