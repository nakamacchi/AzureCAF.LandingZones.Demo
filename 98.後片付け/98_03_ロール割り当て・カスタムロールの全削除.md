...
スクリプト実行の注意点として、**カスタムロールの削除処理（本スクリプトの最後の部分）は一度だけ実行するのではなく、何度か繰り返して実施してください**。これはカスタムロールの実装の一部にキャッシュが使われており、タイムラグが発生したり、一度の削除命令だけでは速やかに削除されない場合があるためです。削除処理を、ロール定義が完全に消えるまで複数回繰り返すようにしてください。（完全に消えるまで 10 分程度かかることが多いです。）
