# Azure Policy Add-on と Defender センサーの有効化

- Azure Policy Add-on → --addons azure-policy
- Defender センサー → --enable-defender
...
TEMP_RP_NAMES="Microsoft.PolicyInsights Microsoft.OperationalInsights"
TEMP_FEATURE_NAMES="\
"
...
az aks enable-addons --resource-group ${TEMP_RG_NAME} --name ${TEMP_AKS_CLUSTER_NAME} --addons azure-policy --workspace-resource-id $TEMP_LAW_RES_ID

az aks update --enable-defender --resource-group ${TEMP_RG_NAME} --name ${TEMP_AKS_CLUSTER_NAME}
