...
for i in ${VDC_NUMBERS}; do
  ...
  TEMP_LAW_ID=$(az monitor log-analytics workspace show --workspace-name ${TEMP_LAW_NAME} --resource-group ${TEMP_RG_NAME} --query customerId -o tsv)
  TEMP_DCE_CONFIG_EP=$(az monitor data-collection endpoint show --name ${TEMP_DCE_NAME} --resource-group ${TEMP_RG_NAME} --query configurationAccess.endpoint -o tsv)
  TEMP_DCE_CONFIG_FQDN=${TEMP_DCE_CONFIG_EP:8}
  TEMP_DCE_INGEST_EP=$(az monitor data-collection endpoint show --name ${TEMP_DCE_NAME} --resource-group ${TEMP_RG_NAME} --query logsIngestion.endpoint -o tsv)
...
az network firewall policy rule-collection-group collection add-filter-collection \
	--resource-group ${TEMP_MAIN_RG_NAME} --policy-name ${TEMP_MAIN_FWP_NAME} --rcg-name "DefaultApplicationRuleCollectionGroup" \
	--name "Azure Portal" --rule-type ApplicationRule --collection-priority 40000 --action Allow \
