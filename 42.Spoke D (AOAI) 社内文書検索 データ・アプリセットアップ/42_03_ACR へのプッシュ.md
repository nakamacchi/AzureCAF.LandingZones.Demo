...
- **vm-mnt-XXX の WSL Ubuntu にログインし**、以下を実行します。
  - これにより、az cli がインストールされます。
  - az cli に対してシステム割り当て MID でログインします。

```bash

# vm-mnt-XXX の WSL 上で実行し、az cli を利用できるようにしておく（az acr login コマンドを利用できるようにするため）
...
azrefadmin@vm-mnt-eus:~/src/webapp$ sudo az login --identity
[
  {
...
azrefadmin@vm-mnt-eus:~/src/webapp$ sudo az acr login --name "acrspoked07541eus.azurecr.io"
...
azrefadmin@vm-mnt-eus:~/src/webapp$ sudo docker tag app:latest acrspoked07541eus.azurecr.io/aoai.sample4:20230810-095303
...
azrefadmin@vm-mnt-eus:~/src/webapp$ sudo docker push acrspoked07541eus.azurecr.io/aoai.sample4:20230810-095303
The push refers to repository [acrspoked07541eus.azurecr.io/aoai.sample4]
